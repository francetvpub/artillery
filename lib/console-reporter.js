...
ConsoleReporter.prototype.done = function done(data) {
  ...
  if (latencies.length > 0) {
    let reformatLatencies = latencies.map((latencie) => {
      console.log(this.blockTest + '_start: ', (latencie[5][1].replace('Z', '')));
      console.log(this.blockTest + ': ', (latencie[2] / 1000000));
      return {
        Data : JSON.stringify({
          phase: this.blockTest,
          url: latencie[4],
          statusCode: latencie[3],
          responseTime: (latencie[2] / 1000000),
          datetz: (latencie[5].join(' ')).replace('Z', ''),
          date: latencie[5][0],
          time: ((latencie[5][1]).split('.'))[0]
        })
      }
    });

    reformatLatencies.forEach((latency) => {
      console.log('ResultItem: ', JSON.stringify(latency));
    });
  }
